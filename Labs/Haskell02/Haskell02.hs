...
identifier = do { d <- many alphaNum; return d }-- #TODO replace me
...
item = do { ident <- identifier;
            symbol ":";
            num <- integer;
            return (ident,num) } -- #TODO replace me
...
items p = do { xs <- sepBy p (symbol ",");
               return xs }-- #TODO replace me
...
intDict = do { c <- parens (items item);
               return c }-- #TODO replace me
...
parseDict ss = case parse intDict "" ss of
                    Left err -> error $ "Parse error" ++ show err
                    Right se -> Map.fromList $ se -- #TODO replace me
